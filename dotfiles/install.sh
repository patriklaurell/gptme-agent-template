...
echo ""
echo "Next steps:"
echo "  1. Customize forbidden repo patterns in .config/git/hooks/pre-commit"
echo "  2. Set up systemd services (optional):"
echo "     cd .config/systemd/user/"
echo "     cp agent-autonomous.service.template ${AGENT_NAME}-autonomous.service"
echo "     cp agent-autonomous.timer.template ${AGENT_NAME}-autonomous.timer"
echo "     # Edit to replace {{AGENT_NAME}}, {{AGENT_HOME}}, {{AGENT_REPO}}"
echo "     # Then: systemctl --user enable ${AGENT_NAME}-autonomous.timer"
echo ""
echo "To test hooks:"
